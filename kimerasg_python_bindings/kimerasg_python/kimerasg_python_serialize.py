...
PATH_TO_KIMERASG = path.dirname(getcwd())+"/kimerasg_python/vxblx_files/"
SCENE_GRAPH_FILE_NAME = "goseek_scene_graph"
# SCENE_GRAPH_FILE_NAME = "office_scene_graph_esdf_slice_1-3"


DOWNSAMPLED_PLACE_NODE_NUM = 100 #number place nodes to add initially for intra layer connections
MAX_PLACE_NODES = 2000 #maximum number of place nodes
...
def parse_scene_graph(kimerasg):
    ...
    for room_id in room_layer_nodes:
        ...
        semantic_label = ROOM_ID_TO_SEMANTIC_LABEL[room_id] #"room"
...
def load_kimerasg():
    ...
    sg.load_scene_graph(PATH_TO_KIMERASG + SCENE_GRAPH_FILE_NAME + '.vxblx', kimerasg)
    return kimerasg
...
if __name__ == "__main__":
    ...
    with open('pickled_output_graphs/' + SCENE_GRAPH_FILE_NAME + 'test.pkl', 'wb') as output:
        pickle.dump(output_graph, output, protocol=2)
    print("begun removing layer")
    graph_without_places = remove_layer(output_graph, NodeType.place)
    print("finished removing layer")
    with open('pickled_output_graphs/' + SCENE_GRAPH_FILE_NAME + '_test_remove_layer.pkl', 'wb') as output:
