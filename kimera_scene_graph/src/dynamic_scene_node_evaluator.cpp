...
DynamicSceneGraphEvaluator::DynamicSceneGraphEvaluator(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
    ...
    optimized_centroid_check_srv_ = nh_private_.advertiseService("compute_optimized_error", &DynamicSceneGraphEvaluator::errorServiceCall, this);
    human_sub_ = nh_private_.subscribe<graph_cmr_ros::SMPLList>(human_topic_, 1, &DynamicSceneGraphEvaluator::humanCallback, this);
}


void DynamicSceneGraphEvaluator::humanCallback(const graph_cmr_ros::SMPLList::ConstPtr& msg){
    if (msg->human_meshes.size() > 0){
        std::vector<tf::StampedTransform> gt_human_transforms;
        ros::Time time(msg->human_meshes[0].header.stamp.sec, msg->human_meshes[0].header.stamp.nsec);

        // Ensure that we could get all transforms before adding them to ground-truth.
...
void DynamicSceneGraphEvaluator::computeOptimizedError(kimera_scene_graph::CentroidErrorRequest::Response& res){
