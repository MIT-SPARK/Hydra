...
PlacesRoomConnectivityFinder::PlacesRoomConnectivityFinder(
...
void PlacesRoomConnectivityFinder::findPlacesRoomConnectivity(
  ...
  utils::convertLayerToSkeleton(*places_layer, &non_segmented_places_skeleton);
  ...
  for (const std::pair<NodeId, SceneGraphNode>& room :
    ...
    while (edge_ids.size() > 0) {
      ...
      SceneGraphNode& vtx_end = place_layer->getNodeMutable(edge.end_node_id_);
    ...
    CHECK_NOTNULL(places_layer);
    utils::convertLayerToSkeleton(*places_layer,
                                  &semi_segmented_places_skeleton);
    visualization_msgs::MarkerArray marker_array;
    ...
    if (!place_node.hasParent()) {
      ...
      for (const std::pair<EdgeId, SceneGraphEdge>& edge_kv :
        ...
        if (edge.end_node_id_ != vertex_id) {
          ...
          // neighbor!
          neighbor_vertex = &places_layer->getNodeMutable(edge.start_node_id_);
        } else {
  ...
  CHECK_NOTNULL(places_layer);
  utils::convertLayerToSkeleton(*places_layer, &segmented_places_skeleton);
