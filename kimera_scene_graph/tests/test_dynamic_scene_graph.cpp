...
TEST_F(DynamicSceneNodeTest, testGraphConnection) {
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    EXPECT_EQ(d_graph.pose_graphs_.size(), d_graph.graph_priors_.size())
        << "STILL -- graphs not aligned with priors -- iter: " << i;

    // Ensure that there are the correct number of graphs.
    EXPECT_EQ(d_graph.pose_graphs_.size(), 1u)
        << "STILL -- too many pose graphs -- iter: " << i;

    // Ensure that there are the right number of nodes in the graph.
    EXPECT_EQ(d_graph.last_poses_.size(), 1u)
        << "STILL -- too many last poses -- iter: " << i;
    EXPECT_EQ(d_graph.counts_[0], i + 1)
        << "STILL -- too many poses in chain -- iter: " << i;
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    // Ensure internal variables that are meant to be updated together are.
    EXPECT_EQ(d_graph.pose_graphs_.size(), d_graph.graph_priors_.size())
        << "MOVING -- graphs not aligned with priors -- iter: " << i;

    // Ensure that there are the correct number of graphs.
    EXPECT_EQ(d_graph.pose_graphs_.size(), 1)
        << "MOVING -- too many pose graphs -- iter: " << i;

    // Ensure that there are the right number of nodes in the graph.
    EXPECT_EQ(d_graph.last_poses_.size(), 1)
        << "MOVING -- too many last poses -- iter: " << i;
    EXPECT_EQ(d_graph.counts_[0], 3 + i)
        << "MOVING -- incorrect poses in chain -- iter: " << i;
  }
  ...
  for (size_t i = 0; i < 2; i++) {
    ...
    // Ensure internal variables that are meant to be updated together are.
    EXPECT_EQ(d_graph.pose_graphs_.size(), d_graph.graph_priors_.size())
        << "MULTI -- graphs not aligned with priors -- iter: " << i;

    // Ensure that there are the correct number of graphs.
    EXPECT_EQ(d_graph.pose_graphs_.size(), 2 + i)
        << "MULTI -- too many pose graphs -- iter: " << i;

    // Ensure that there are the right number of nodes in the graph.
    EXPECT_EQ(d_graph.last_poses_.size(), 2 + i)
        << "MULTI -- too many last poses -- iter: " << i;
    for (size_t j = 0; j <= i; j++) {
      if (j == 0)
        EXPECT_EQ(d_graph.counts_[j], 4)
            << "multi -- incorrect poses in chain -- iter: " << i;
      else
        EXPECT_EQ(d_graph.counts_[j], 1)
            << "multi -- incorrect poses in chain -- iter: " << i;
    }
  }
}
