...
TEST_F(DynamicSceneNodeEvaluatorTest, testVariance){
    ...
    EXPECT_TRUE(abs(variance - online_variance) < 0.3) << "Variance: "  << variance << "  Online Variance: " << online_variance;
}
...
TEST_F(DynamicSceneNodeEvaluatorTest, testSmallestCentroidDistance){
    ...
    node.pose_ = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(1.0, 1.0, 1.0));
    ...
    tf1.setData(tr1);
    tf2.setData(tr2);
    dsn_eval.time_to_gt_[node.msg_.header.stamp.toSec()] = std::vector<tf::StampedTransform>({tf1, tf2});
    EXPECT_TRUE(abs(dsn_eval.smallestCentroidDistance(node) - 1.0) < 0.0000001) << dsn_eval.smallestCentroidDistance(node);
}
...
TEST_F(DynamicSceneNodeEvaluatorTest, testErrorServiceCall){
    ...
    transform.setOrigin(tf::Vector3(smpl.centroid[0] + 1.0, smpl.centroid[1] + 1.0, smpl.centroid[2] - dsg_eval.human_height_ / 2.0) );
    transform.setRotation(tf::Quaternion(quat.x(), quat.y(), quat.z(), quat.w()) );

    br.sendTransform(tf::StampedTransform(transform, base_time, "world", "object_0"));
    ...
    // TODO (argupta) check original nodes have correct values + optimized is less
