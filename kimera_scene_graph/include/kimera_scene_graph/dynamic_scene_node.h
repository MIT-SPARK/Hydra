...
typedef std::vector<std::vector<DynamicSceneNode>> DynamicNodeList;
typedef std::vector<std::pair<gtsam::Pose3, DynamicSceneNode*>> CentroidNodeList;
 
class DynamicSceneGraph : public SceneGraph {
 public:
    DynamicSceneGraph(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
 ...
 public:
    ...
    /**
     * @brief humanCallback populates the DynamicSceneGraph with humans from SMPLList messages.
     * @param msg: The SMPLList message containing the detected human SMPLs and their centroids.
     */
    void humanCallback(const graph_cmr_ros::SMPLList::ConstPtr& msg);


    /**
     * @brief visualizePoseGraphs publishes the centroids of the humans and the edges connecting them
     * for each human individually. It additionally publishes all the centroids and all the edges.
     * @param serialize: If true, serializes the messages to the serialization_dir_ specified in the launch file.
     */
    void visualizePoseGraphs(bool serialize=false);


    /**
     * @brief visualizeJoints visualizes the skeleton edges for each human, the joint vertices, and an SMPL list with the
    ...
    /**
     * @brief getNodes gets the nodes from the graph in lists based on pose graph membership.
     */
    inline DynamicNodeList* getNodes(){
        return &last_poses_;
    }

    /**
     * @brief optimizedGraph prune and optimize the pose graph.
     * @param optimized_nodes: An empty vector to be filled with the resulting nodes.
     */
    ...
    /**
     * @brief dynamicSceneNodeFromSMPL converts an SMPL into a DynamicSceneNode representation.
     * @param mesh: The SMPL message to be converted into a node. 
     * @param node: A node object to be filled with the resulting DynamicSceneNode.
     */
    void dynamicSceneNodeFromSMPL(graph_cmr_ros::SMPL& mesh, DynamicSceneNode& node);

    /**
     * @brief checkDynamicFeasibility checks whether the distance between centroids is possible given the rate.
     * @param scene_node: The node to be checked as a feasible node in the graph.
     * @param last_node: The last node currently in the graph to be checked against.
     * @return true if the distance is feasible or if filter_centroid is set to false in the launch file.
     */
    ...
    /**
     * @brief checkMeshFeasibility checks whether the largest distance between joints is possible given the rate.
     * @param scene_node: The node to be checked as a feasible node in the graph.
     * @param last_node: The last node currently in the graph to be checked against.
     * @return true if the distance is feasible or if filter_centroid is set to false in the launch file.
     */
 ...
 private:
    ...
    /**
     * @brief deserializeAndPublish deserializes messages from the deserialization_file
     *  and publishes them.
     */
    bool deserializeAndPublish();

    /**
     * @brief serialize the messages to a bag file in the serialization_dir_ named based on the number
     *  of pose graphs.
     */
     ...
     * @param create_dirs: If true, makes the directories if they do not already exist.
     * @return true if the directory exists or was successfully made, false otherwise.
     */
    ...
    inline void setupEdgeMarker(visualization_msgs::Marker& edges, std::vector<float>& color);
