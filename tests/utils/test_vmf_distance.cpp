...
TEST(VmfDistance, KappaParityWithPythonReferenceAtRbar07) {
  // From vggt_slam/places/graph.py:estimate_vmf_kappa(r_bar=0.7, d=1024, k=10):
  // kappa = 0.7 * (1024 - 0.49) / (1 - 0.49) = 0.7 * 1023.51 / 0.51 = ~1404.66
  // Clipped to kappa_max=500 -> 500. Test at kappa_max=5000 to avoid the clip.
  const float k = estimateVmfKappa(0.7f, 1024, 10u, /*kappa_max=*/5000.0f);
  // Compute expected manually to keep this test self-checking.
