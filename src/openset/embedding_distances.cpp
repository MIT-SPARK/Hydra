...
float CosineDistance::dist(const Eigen::VectorXf& lhs,
                           const Eigen::VectorXf& rhs) const {
  // map [-1, 1] to [0, 2]
  return 1.0f - score(lhs, rhs);
}

float CosineDistance::score(const Eigen::VectorXf& lhs,
                            const Eigen::VectorXf& rhs) const {
  const auto divisor = std::max(lhs.norm() * rhs.norm(), config.tolerance);
  return lhs.dot(rhs) / divisor;
}

float L2Norm::dist(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
  return (lhs.normalized() - rhs.normalized()).norm();
}

float L2Norm::score(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
  // map [2, 0] to [0, 1]
  return 1.0f - 0.5f * dist(lhs, rhs);
}

float L1Norm::dist(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
  return (lhs.normalized() - rhs.normalized()).lpNorm<1>();
}

float L1Norm::score(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
  // map [2, 0] to [0, 1]
  return 1.0f - 0.5f * dist(lhs, rhs);
}
...
float LerfScore::dist(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
  // technically the softmax isn't a dist
  return 1.0f - score(lhs, rhs);
}

float LerfScore::score(const Eigen::VectorXf& lhs, const Eigen::VectorXf& rhs) const {
